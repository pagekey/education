...
def partition(arr, low, high):			# We already know low < high, so there's at least 2 elements in arr
	...
	while i < high - 1:					# For each element, EXCLUDING the pivot, we will check if less than pivot.
		if arr[i] < pivot then			# We found something to put behind the wall!
			swap(arr, i, wall)			# Swap element at i to be behind the wall.
			i += 1						# Move the wall up by one.
	swap(arr, i, high)					# Swap the high index (the pivot index) into it's new place. All to left of it is less, all to right gte
